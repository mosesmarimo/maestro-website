...
export const FAQ_SEED: FaqEntry[] = [
  ...
  },
  {
    ...
    answerHtml:
      "<p>Yes. The whole blueprint — the orchestration studio and the agent platform layer — is now implemented and ships in the current Alpha build: the canvas and orchestrator with event-log replay, the live agent map, the MRGD engine, real media generation, persistent memory, self-authored skills, the messaging gateway, natural-language schedules, sandboxed Docker/SSH backends, browser automation with vision, multi-agent rooms, the web console, and trajectory export.</p><p>It is an <strong>Alpha</strong>: builds are unsigned and rough in places, so expect bugs. Signed and notarized installers, stability, and production hardening arrive with Beta. See the <a href=\"/#roadmap\">roadmap</a>.</p>",
  },
  {
